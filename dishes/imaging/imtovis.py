...
def imtovis(image):
    ...
    uu = fftshift(fftfreq(image.x.size, (image.x[1] - image.x[0]) * units.arcsec.to(units.radian)))
    vv = fftshift(fftfreq(image.y.size, (image.y[1] - image.y[0]) * units.arcsec.to(units.radian)))
